 ...
 *  创建弹幕
 *
 *  @param bullchtaModel    弹幕内容
 *  @param trajectory 弹道位置
 */
- (void)createBulletComment:(DongwangbulletChtaModel *)bullchtaModel trajectory:(Trajectory)trajectory {
    if (self.bStopAnimation) {
        return;
    }
    //创建一个弹幕view
    BulletView *view = [[BulletView alloc] initWithContent:bullchtaModel];
    //设置运行轨迹
    ...
    view.moveBlock = ^(CommentMoveStatus status) {
        ...
        switch (status) {
            ...
            case Enter: {
                ...
                DongwangbulletChtaModel *chatModel = [myself nextComment];
                if (chatModel) {
                    [myself createBulletComment:chatModel trajectory:trajectory];
                } else {
                    //说明到了评论的结尾了
                }
                break;
...
- (void)initBulletCommentView {
    ...
    for (int i = 3; i > 0; i--) {
        DongwangbulletChtaModel * chatModel  = [self.tmpComments firstObject];
        if (chatModel) {
            ...
            [self createBulletComment:chatModel trajectory:trajectory];
        } else {
            //当弹幕小于三个，则跳出
            break;
        }
...
- (DongwangbulletChtaModel *)nextComment {
    DongwangbulletChtaModel * chatModel  = [self.tmpComments firstObject];
    if (chatModel) {
        [self.tmpComments removeObjectAtIndex:0];
    }
