...
+ (void)load {
    ...
    for (NSUInteger index = 0; index < sizeof(selectors) / sizeof(SEL); ++index) {
        ...
        SEL swizzledSelector = NSSelectorFromString([@"ch_" stringByAppendingString:NSStringFromSelector(originalSelector)]);
    ...
    if (self.showNoDataNotice) {
        ...
        if (rowCount == 0) {
            if (self.customNoDataView) {
                ...
                self.backgroundView = [self ch_defaultNoDataView];
        } else {
...
- (UIView *)ch_defaultNoDataView {
    ...
    self.defaultNoDataView = ({
        ...
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(ch_tapDefalutNoDataView:)];
...
- (void)layoutDefaultView:(UIView *)defaultView {
    ...
    UIImage *image = self.defaultNoDataImage ? : [UIImage imageNamed:@"blank"];
...
- (void)ch_tapDefalutNoDataView:(UITapGestureRecognizer *)tap {
...
- (UILabel *)defaultNoDataNoticeLabel {
    ...
    if (!label) {
        ...
        label.font = [UIFont systemFontOfSize:RealWidth(14)];
        label.textAlignment = NSTextAlignmentCenter;
        label.textColor = LGDGaryColor;
...
- (UIImageView *)defaultNoDataNoticeImageView {
    ...
    if (!imageView) {
        ...
        imageView.image = self.defaultNoDataImage ? : [UIImage imageNamed:@"blank"];
